...
RequestHandler.prototype.onData = function(id, data, callback) {
  try {
    ...
    this.store.pushToSendQ(id, {type: 'error', reason: 'parse', advice: 'reconnect'});
    return;
  }
...
RequestHandler.prototype.payload = function(req, res, reqData, msgs) {
  ...
  if (reqData.query.i && jsonpPollingRe.test(reqData.query.i)) {
    head = 'io.j[' + reqData.query.i + '](';
  }

  msgs = msgs ? head + JSON.stringify(msgs) + foot : '';

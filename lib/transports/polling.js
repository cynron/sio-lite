...
RequestHandler.prototype.handleRequest = function(req, res, data) {
  if (req.method === 'POST') {
    this.handlePost(req, res, data);
  } else {
    this.handleGet(req, res, data);
  }
};
...
RequestHandler.prototype.handleGet = function(req, res, data) {
  var self = this;
  ...
  self.store.pushToRecvQ(data.id, noopPacket, function(err) {
    if (err) {
      ...
      req.socket.once('close', exHandler);
      req.socket.once('end', exHandler);
      req.socket.once('error', errorHandler);
      ...
      self.store.getFromSendQ(data.id, function(isTimeout, packets) {
        ...
        self.payload(req, res, data, packets);
      });
    }
  });
};
...
RequestHandler.prototype.handlePost = function(req, res, data) {
  var buffer = null 

...
XHRPolling.init = function(manager) {
  console.log('init manager---:', manager);
  console.log('init store------:', manager.store);
...
XHRPolling.prototype.onConnect = function() {
  ...
  XHRPolling.handler.payload(this.req, res, {type: 'connect'});
}
...
RequestHandler.prototype.payload = function(req, res, data) {
  ...
  for(var i = 0; i < data.length; ++i) {
    packets.push(parser.encodePacket(data[i]));
  }

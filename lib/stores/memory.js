...
Client.prototype.pushToRecvQ = function(msgs, cb) {
  this.recvQ = this.recvQ.concat(msgs);
  this.emit('recv');
  util.invoke(cb); 
}
...
Client.prototype.pushToSendQ = function(msgs, cb) {
  this.sendQ = this.sendQ.concat(msgs);
  console.log('---------sendQ:', this.sendQ);
  this.emit('send');
  util.invoke(cb);
}

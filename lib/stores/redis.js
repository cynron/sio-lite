...
Redis.prototype.onHandshaken = function(id, data, cb) {
  ...
  this.cmd.set('handshaken:' + id, this.pack(data), function(err) {
    if (err) {
      self.log.error('add handshaken data error:' + err);
      util.invoke(cb, err);
    } else {
      // 30 seconds for handshaken expiration
      var timeout = self.manager.get('handshake expiration');
      self.cmd.expire('handshaken:' + id, timeout, cb);
    }
  });
};
...
Redis.prototype.onConnected = function(id, cb) {
  ...
  var persistentHandshaken = function(cb) {
    self.cmd.persist('handshaken:' + id, function(err) {
      if (err) {
        self.log.error('persist handshaken data err:' + err);
      }
      util.invoke(cb, err);
    });
  };
...
Redis.prototype.destroy = function() {
  ...
  this.pub.stream.destroy();
  this.sub.stream.destroy();
  this.cmd.stream.destroy();
};
...
Redis.prototype.destroyConnection = function(id, timeout) {
  if (timeout && typeof timeout === 'number') {
    this.cmd.expire('handshaken:' + id, timeout);
    this.cmd.expire('connected:' + id, timout);
    this.cmd.expire('sendq:' + id, timout);
    this.cmd.expire('recvq:' + id, timout);
  } else {
...
Redis.prototype.getFromRecvQ = function (id, cb) {
  ...
  this.cmd.expire('handshaken:' + id, timeout, function(err) {
    if (err) {
      self.log.error('expire handshaken data error:' + err);
      util.invoke(cb, err);
    } else {
      self.cmd.expire('connected:' + id, timeout, function(err) {
        if (err) {
          self.log.error('expire connected error:' + err);
          util.invoke(cb, err);
        } else {
          self.getFromQueue('recvq:' + id, timeout, cb);
        }
      });
    }
  });

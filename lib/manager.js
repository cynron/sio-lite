...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (undefined != data.query.disconnect) {
    this.store.client[id].pushToRecvQ({type: 'disconnect'}, function() {
  ...
  var handleHandshaken = function(handshaken) {
    if (handshaken) {
      ...
      var socket = self.socket(id, true);
      self.store.onConnected(id, function() {
        self.emit('connection', socket);
        Transport.payload(req, res, {type: 'connect'});
      });
    } else {
  ...
  var handleConnected = function(connected) {
    if (connected) {
      ...
      this.store.handshaken(id, handleHandshaken);
    }
  }

  this.store.isConnected(id, handleConnected);
}
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      self.store.onHandleshaken(id, function() {
        res.end(hs);
      });
